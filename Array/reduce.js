...
const userTodos = todos.reduce((acc, todo, index, array) => {
...
users.map((user,index,array) => {
    ...
    user['todos'] = userTodos.hasOwnProperty(key) ? userTodos[key] : [];
});
console.log(users);
...
for(let i=0; i<todos.length; i++){
    const value = todos[i];
    const key = `user${todos[i].userId}`;
    if(!userToDo.hasOwnProperty(key)){
        userToDo[key] = [];
    }
    userToDo[key].push(value);
}
